# 在 Linux 上训练一个定制的 YOLOv4 对象检测器

> 原文：<https://medium.com/geekculture/train-a-custom-yolov4-object-detector-on-linux-49b9114b9dc8?source=collection_archive---------2----------------------->

## (初学者教程)

# 我在 YouTube 上的视频！

# 在本教程中，我们将在 Linux(Ubuntu 18.04)系统上使用 YOLOv4 和 Darknet 训练一个自定义的对象检测器来进行遮罩检测

![](img/4ab269354456b023622c1379902ae8ca.png)

# 按照这 10 个步骤使用 YOLOv4 训练一个物体检测器

(但首先✅Subscribe 到我的 YouTube 频道👉🏻[https://bit.ly/3Ap3sdi](https://bit.ly/3Ap3sdi)😁😜)

1.  [在桌面上创建 ***yolov4*** 和 ***训练*** 文件夹](#c367)
2.  [打开命令提示符，导航到“ **yolov4** ”文件夹](#be3a)
3.  [创建并复制**darknet.exe**文件](#e8a4)
4.  [创建&将我们训练需要的文件(即 **obj** 数据集文件夹、 **yolov4-custom.cfg** 、 **obj.data** 、 **obj.names** 、 **process.py** 文件)复制到你的***yolov 4***dir](#cc35)
5.  [将 ***yolov4*** 目录下的“ **yolov4-custom.cfg** ”、“ **obj.data** ”、“ **obj.names** ”、“ **process.py** ”文件和“**obj**dataset”文件夹从***yolov 4***目录下复制到 ***darknet*** 目录下](#f775)
6.  [运行 **process.py** python 脚本创建**train . txt**&**test . txt**文件](#5d4d)
7.  [下载预先训练好的 **YOLOv4** 重量](#fc1c)
8.  [训练探测器](#ca18)
9.  [检查性能](#f83b)
10.  [测试您的自定义对象检测器](#fbe0)

# ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~开始吧！！~~~~~~~~~~~~~~

![](img/01436089250122521bca4deeb428d3d3.png)

# 1)创建“yolov4”和“training”文件夹

在桌面上创建一个名为 ***yolov4*** 的文件夹。接下来，在 ***yolov4*** 文件夹内创建另一个名为 ***training*** *的文件夹。这是我们将保存训练权重的地方(该路径在我们稍后将上传的 ***obj.data*** 文件中有提及)*

![](img/74fe8a1f6e4601a4066fb6b0a61aad38.png)

# 2)打开终端并导航到“yolov4”文件夹

```
#Navigate to /Desktop/yolov4 folder using the following command in terminal or just right-click and open it in terminal.**cd Desktop/yolov4/**
```

![](img/ee54b26fee9cce961ecf643a9dde3a5e.png)

# 3)创建并复制暗网可执行文件

创建包含 **darknet** 可执行文件的 ***darknet*** 文件夹，并复制到 ***yolov4*** 文件夹中。要了解如何使用 make 创建包含 darknet 可执行文件的 darknet 文件夹，请访问 [**本博客**](https://techzizou007.medium.com/yolov4-darknet-installation-and-usage-on-your-system-windows-linux-8dec2cea6e81) 。按照所有的步骤在 Linux 上安装 YOLO-暗网。
...
以上过程完成后，将我们从中获取的 ***darknet*** 文件夹复制到你桌面上的 ***yolov4*** 文件夹。

![](img/51807ff0ee2c413afcd3f452474e11dc.png)
...
![](img/d8f17d1e34d026f413833ceae2eab77b.png)

你可以使用任何软件来贴标签，比如 [**标签**](https://github.com/tzutalin/labelImg#labelimg) 工具。

![](img/0fce106ff83424fa932ddec6172dc354.png)

我使用一个名为 **OpenLabeling** 的开源标签工具，它有一个非常简单的 UI。

![](img/14f8828b32ae6f1468484fd91a209f50.png)
...
![](img/66ea54d32578eabda1c96be416c2d310.png)

# 4(a)创建带标签的自定义数据集“obj”文件夹，并将其复制到“yolov4”文件夹

把所有输入的图像”。jpg "文件及其对应的 YOLO 格式标注"。txt "文件夹中的文件名为 ***obj*** 。将其复制到**yolov 4文件夹中。**

![](img/aabf621c3bc027369c1605aaf78f119f.png)

# 4(b)创建您的自定义配置文件，并将其复制到“yolov4”文件夹中

从 ***darknet/cfg*** 目录*，*下载***yolov 4-custom . CFG***文件，对其进行修改，并复制到 ***yolov4*** 目录。

你也可以从官方 [AlexeyAB Github](https://www.github.com/AlexeyAB/darknet) 下载定制配置文件。

**您需要在您的自定义配置文件中进行以下更改:**

![](img/ce6effcf52a44f2d545ba41efbb9d15e.png)
...
![](img/fc408c5dc98ed62f6fff1efbd90c5269.png)

*   在每个[yolo]层之前的 **3** **【卷积】**中，将[filters=255]更改为 filters=(classes + 5)x3，请记住，它必须是每个[yolo]层之前的最后一个[卷积]。
...
![](img/f8194ddac5065a63efedf2ca03fe71aa.png)

# 4(d)将<process.py>脚本文件复制到你的 yolov4 文件夹中</process.py>
...
![](img/820ff543f6b295db7bc93ec7667fde34.png)

# 5)将所有文件从'`yolov4'`目录复制到' yolov4/darknet '目录

你可以通过文件管理器手动完成所有这些步骤。

首先，清理***CFG***&***data***文件夹。删除 ***data*** 文件夹中的所有文件，除了 ***labels*** 文件夹，该文件夹是在检测框上写入标签名称所必需的，同时删除 ***cfg*** 文件夹中的所有文件，因为在 ***yolov4*** 主文件夹中已经有了我们的自定义配置文件。
...
![](img/5a295c98943ae4ad1455cd60654a7b73.png)

**或**

如果你是一个终端极客，使用终端完成上述步骤🤓。将当前工作目录更改为**/Desktop/yolov 4**/**darknet**
...
```
**find ./data -mindepth 1 ! -regex '^./data/labels(/.*)?' -delete****rm -rf cfg/
mkdir cfg**
```
...
当前工作目录为**/Desktop/yolov 4/darknet**。
...
上面的 **process.py** 脚本创建了两个文件 **train.txt** 和 **test.txt** ，其中 **train.txt** 的路径指向 90%的图像，而 **test.txt** 的路径指向 10%的图像。这些文件如下所示。

![](img/18e9a6aa2d9a68131e6fc03e605ed723.png)

# 7)下载并复制预训练的 YOLOv4 重量到“yolov4/darknet”目录。

当前工作目录是**/Desktop/yolov 4/darknet**

这里我们用迁移学习。我们没有从头开始训练模型，而是使用预先训练的 YOLOv4 权重，这些权重已经被训练到 137 个卷积层。从这里下载 YOLOv4 预训练重量文件[，并将其复制到您的 ***darknet*** 文件夹中。](https://github.com/AlexeyAB/darknet/releases/download/darknet_yolo_v3_optimal/yolov4.conv.137)
...
![](img/cecbe06752eebfdc202e5b63abcc531f.png)
...
***注意:**根据您在 cfg 文件中的设置，在上面的命令中将细分的第一个值设置为 16 / 32 / 64。*

## *对图像运行检测器*

*将图像上传到桌面上名为 ***mask_test_images*** 的文件夹。*
...
*这样的输出被保存为 ***darknet*** 文件夹中的【predictions.jpg】。您可以将其复制到任何输出文件夹，并用不同的名称保存。我将它作为 **output_image.jpg** 保存在***mask _ test _ images***文件夹本身中。*

```
***cp predictions.jpg ../../mask_test_images/output_image1.jpg***
```

*![](img/cd759e018a4696cd24989dbae9cf63e8.png)**![](img/94e3e262c985843263a0194a93240ca4.png)*
...
*![](img/9905263a55a6d5a7477272ef2e51b4b2.png)*
...
*![](img/41fb7b45e42ef279ea8d1d97ebd5645f.png)*
...
*![](img/b9cd6bb7d4730bf6df4da3e81386da65.png)*

# *GitHub 链接*

*我已经在下面的 GitHub 链接上上传了我的自定义掩膜数据集和在 Windows 中训练自定义 YOLOv4 检测器所需的所有其他文件。*
...
*   *[纸约洛夫 4](https://arxiv.org/abs/2004.10934)*
...
*![](img/1fda33599be85032b98e68dbc7645718.png)*